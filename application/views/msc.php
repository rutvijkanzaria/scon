

<!--Page Header-->
<section class="page_header child_nursing padding-top">
  <div class="container">
    <div class="row">
      <div class="col-md-12 page-content">
        <h1>Post Graduation - M.Sc Nursing</h1>
        <div class="page_nav">
          <span>You are here:</span> <a href="<?php echo base_url() ?>">Home</a> <span><i class="fa fa-angle-double-right"></i>Post Graduation - M.Sc Nursing</span>
        </div>
      </div>
    </div>
  </div>
</section>
<!--Page Header-->


<!-- Courses -->
<section id="course_all" class="padding-bottom-half padding-top">
  <div class="container content-container">
    <div class="row">
      <div class="col-sm-10 col-sm-offset-1 course_detail wow fadeIn" data-wow-delay="400ms">
        <img src="<?php echo base_url() ?>images/academics/msc.jpg" alt="Course" class=" border_radius img-responsive bottom15 hero-image">
        <h3 class="top30 bottom20">Post Graduation - M.Sc Nursing</h3>
        <p class="bottom25">Saveetha College of Nursing offering PG course from 1995 in all the branches. We focus on developing their abilities managerial, leadership, and research aspects. Post graduate students are exposed to a variety of teaching and learning methods including seminars, discussions, problem-solving activities. It helps them to experience and transfer their learning in order to develop the higher level of knowledge and skills required for specialist and advanced practice. In Clinical area we are practicing CBL, Bed side clinic, nursing conference   Inter Professional learning to improve collaboration and the quality of care, helps students to appreciate the importance of personalities and interpersonal skills. Our clinical area is enriched with various recent technologies & treatment modalities in the management of critically ill patients and also it provide opportunities for our students to conduct individual and inter professional project to provide evidence based and high quality patient care. Our learning environment promoting our student to take the role as a as advocate, Administrator, researcher, counsellor and manager.
        </p>
        <h3 class="top30 bottom20">Best Practices</h3>
        <ul class="bottom25">
          <li> 	Inter professional learning and research</li>
          <li> 	Interdisciplinary learning</li>
          <li> 	Action research </li>
        </ul>
        <h3 class="top30 bottom20">Curriculum and Year Plan</h3>
        <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
          <div class="panel panel-default">
            <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
            <div class="panel-heading" role="tab" id="headingOne">
              <h4 class="panel-title">
                  Curriculum
              </h4>
            </div>
          </a>
            <div id="collapseOne" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingOne">
              <div class="panel-body">
                <table class="table table-striped table-bordered table-hover">
                  <tbody>
                    <tr>
                    <td colspan="5">
                    <p><strong>Second Year</strong></p>
                    </td>
                    </tr>
                    <tr>
                    <td colspan="2">
                    <p><strong>S.No</strong></p>
                    </td>
                    <td>
                    <p><strong>Subject</strong></p>
                    </td>
                    <td>
                    <p><strong>Hours Theory</strong></p>
                    <p>&nbsp;</p>
                    </td>
                    <td>
                    <p><strong>Hours Practical </strong></p>
                    </td>
                    </tr>
                    <tr>
                    <td colspan="2">
                    <p>1.</p>
                    </td>
                    <td>
                    <p>Nursing Education</p>
                    </td>
                    <td>
                    <p>150</p>
                    </td>
                    <td>
                    <p>150</p>
                    </td>
                    </tr>
                    <tr>
                    <td colspan="2">
                    <p>2.</p>
                    </td>
                    <td>
                    <p>Advanced Nursing Practice</p>
                    </td>
                    <td>
                    <p>150</p>
                    </td>
                    <td>
                    <p>200</p>
                    </td>
                    </tr>
                    <tr>
                    <td colspan="2">
                    <p>3.</p>
                    </td>
                    <td>
                    <p>Nursing Research &amp; Statistics</p>
                    </td>
                    <td>
                    <p>150</p>
                    </td>
                    <td>
                    <p>100</p>
                    </td>
                    </tr>
                    <tr>
                    <td colspan="2">
                    <p>4.</p>
                    </td>
                    <td>
                    <p><sup>*</sup>Clinical Speciality - I</p>
                    </td>
                    <td>
                    <p>150</p>
                    </td>
                    <td>
                    <p>650</p>
                    </td>
                    </tr>
                    <tr>
                    <td colspan="3">
                    <p><strong>Total Hours</strong></p>
                    </td>
                    <td>
                    <p><strong>600</strong></p>
                    </td>
                    <td>
                    <p><strong>1100</strong></p>
                    </td>
                    </tr>
                    <tr>
                    <td colspan="5">
                    <p><strong>Second Year</strong></p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>1</p>
                    </td>
                    <td colspan="2">
                    <p>Nursing Management</p>
                    </td>
                    <td>
                    <p>150</p>
                    </td>
                    <td>
                    <p>150</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>2</p>
                    </td>
                    <td colspan="2">
                    <p>Nursing Research ( Dissertation)</p>
                    </td>
                    <td>
                    <p>-</p>
                    </td>
                    <td>
                    <p>300</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>3</p>
                    </td>
                    <td colspan="2">
                    <p><sup>*</sup>Clinical Speciality - II</p>
                    </td>
                    <td>
                    <p>150</p>
                    </td>
                    <td>
                    <p>950</p>
                    </td>
                    </tr>
                    <tr>
                    <td colspan="3">
                    <p><strong>Total Hours</strong></p>
                    </td>
                    <td>
                    <p><strong>300</strong></p>
                    </td>
                    <td>
                    <p><strong>1400</strong></p>
                    </td>
                    </tr>
                    </tbody>
                </table>
              </div>
            </div>
          </div>
          <div class="panel panel-default">
            <a class="collapsed" role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">
            <div class="panel-heading" role="tab" id="headingTwo">
              <h4 class="panel-title">
                  Year Plan
              </h4>
            </div>
          </a>
            <div id="collapseTwo" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingTwo">
              <div class="panel-body">
                <table class="table table-striped table-bordered table-hover">
                  <tbody>
                    <tr>
                    <td width="88">
                    <p><strong>MONTH</strong></p>
                    </td>
                    <td width="128">
                    <p><strong>DATE</strong></p>
                    </td>
                    <td width="400">
                    <p><strong>EVENTS</strong></p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="2" width="88">
                    <p>September 2016</p>
                    </td>
                    <td width="128">
                    <p>01.09.2016</p>
                    </td>
                    <td width="400">
                    <p>University Examination Commences for all courses</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>05.09.2016</p>
                    </td>
                    <td width="400">
                    <p>Teacher&rsquo;s Day</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="2" width="88">
                    <p>October</p>
                    <p>2016</p>
                    </td>
                    <td width="128">
                    <p>07.10.2016</p>
                    </td>
                    <td width="400">
                    <p>State Level Workshop - CHN</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>26.10.2016</p>
                    </td>
                    <td width="400">
                    <p>Continuing Nursing Education-MHN</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="3" width="88">
                    <p>November 2016</p>
                    </td>
                    <td width="128">
                    <p>01.11.2016 to 05.11.2016</p>
                    </td>
                    <td width="400">
                    <p>Newborn Week</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>16.11.2016</p>
                    </td>
                    <td width="400">
                    <p>Guest Lecture - FON</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>18.11.2016</p>
                    </td>
                    <td width="400">
                    <p>Founders Day</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="3" width="88">
                    <p>December 2016</p>
                    </td>
                    <td width="128">
                    <p>01.12.2016</p>
                    </td>
                    <td width="400">
                    <p>World AIDS Day - Rally</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>02.12.2016</p>
                    </td>
                    <td width="400">
                    <p>First Internal Examination Commences</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>20.12.2016</p>
                    </td>
                    <td width="400">
                    <p>Guest Lecture - MSN</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="88">
                    <p>January-2017</p>
                    </td>
                    <td width="128">
                    <p>27.01.2017</p>
                    </td>
                    <td width="400">
                    <p>Continuing Nursing Education - MSN</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="3" width="88">
                    <p>February</p>
                    <p>2017</p>
                    </td>
                    <td width="128">
                    <p>03.02.2017</p>
                    </td>
                    <td width="400">
                    <p>National Level Conference - MSN</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>10.02.2016</p>
                    </td>
                    <td width="400">
                    <p>District Level hands on workshop - PDN</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>17.02.2017</p>
                    </td>
                    <td width="400">
                    <p>Guest Lecture- MHN</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="7" width="88">
                    <p>March-2017</p>
                    </td>
                    <td width="128">
                    <p>08.03.2017</p>
                    </td>
                    <td width="400">
                    <p>International Women&rsquo;s day</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>13.03.2017</p>
                    </td>
                    <td width="400">
                    <p>Second Terminal Examination Commences</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>15.03.2017</p>
                    </td>
                    <td width="400">
                    <p>Continuing Nursing Education -CHN</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>18.03.2017</p>
                    </td>
                    <td width="400">
                    <p>Convocation Day</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>22.03.2017</p>
                    </td>
                    <td width="400">
                    <p>In- Service Education</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>29.03.2017</p>
                    </td>
                    <td width="400">
                    <p>Guest lecture-PDN</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>28.03.2017 to</p>
                    <p>30.03.2017</p>
                    </td>
                    <td width="400">
                    <p>Education tour</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="4" width="88">
                    <p>April</p>
                    <p>2017</p>
                    </td>
                    <td width="128">
                    <p>06.04.2017</p>
                    </td>
                    <td width="400">
                    <p>Safe motherhood Day</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>07.04.2017</p>
                    </td>
                    <td width="400">
                    <p>WHO day</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>12.04.2017</p>
                    </td>
                    <td width="400">
                    <p>Guest Lecture&nbsp; -CHN</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>17.04.2017</p>
                    </td>
                    <td width="400">
                    <p>Continuing Nursing Education - OBGN</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="4" width="88">
                    <p>May</p>
                    <p>2017</p>
                    </td>
                    <td width="128">
                    <p>08.05.2017</p>
                    </td>
                    <td width="400">
                    <p>Mother&rsquo;s Day</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>10.05.2017</p>
                    </td>
                    <td width="400">
                    <p>Continuing Nursing Education-PDN</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>12.05.2017</p>
                    </td>
                    <td width="400">
                    <p>Nurses Day</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>15.05.2017 to 27.05.2017</p>
                    </td>
                    <td width="400">
                    <p>Certificate Course on Guidance and Counselling</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="2" width="88">
                    <p>June</p>
                    <p>2017</p>
                    </td>
                    <td width="128">
                    <p>01.06.2017</p>
                    </td>
                    <td width="400">
                    <p>Third Terminal Examination commences</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>17.06.2017</p>
                    </td>
                    <td width="400">
                    <p>Guest Lecture - OBGN</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="4" width="88">
                    <p>July</p>
                    <p>2017</p>
                    </td>
                    <td width="128">
                    <p>11.07.2017</p>
                    </td>
                    <td width="400">
                    <p>Model Examination commences</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>22.07.2017 &amp; 23.07.2016</p>
                    </td>
                    <td width="400">
                    <p>International Nursing Conference</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>26.07.2017</p>
                    </td>
                    <td width="400">
                    <p>Guest lecture</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>29.07.2017</p>
                    </td>
                    <td width="400">
                    <p>Continuing Nursing Education-CHN</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="2" width="88">
                    <p>August</p>
                    <p>2017</p>
                    <p>&nbsp;</p>
                    </td>
                    <td width="128">
                    <p>01.08.2017 to 08.08.2017</p>
                    </td>
                    <td width="400">
                    <p>World Breast Feeding Celebration</p>
                    </td>
                    </tr>
                    <tr>
                    <td width="128">
                    <p>04.08.2017</p>
                    </td>
                    <td width="400">
                    <p>Guest lecture-MHN</p>
                    </td>
                    </tr>
                    </tbody>
                </table>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>
<!-- Courses -->
