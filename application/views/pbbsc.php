

<!--Page Header-->
<section class="page_header child_nursing padding-top">
  <div class="container">
    <div class="row">
      <div class="col-md-12 page-content">
        <h1>P.B.B.Sc  Nursing</h1>
        <div class="page_nav">
          <span>You are here:</span> <a href="<?php echo base_url() ?>">Home</a> <span><i class="fa fa-angle-double-right"></i>P.B.B.Sc  Nursing</span>
        </div>
      </div>
    </div>
  </div>
</section>
<!--Page Header-->


<!-- Courses -->
<section id="course_all" class="padding-bottom-half padding-top">
  <div class="container content-container">
    <div class="row">
      <div class="col-sm-10 col-sm-offset-1 course_detail wow fadeIn" data-wow-delay="400ms">
        <img src="<?php echo base_url() ?>images/academics/bsc.jpg" alt="Course" class=" border_radius img-responsive bottom15 hero-image">
        <h3 class="top30 bottom20">P.B.B.Sc  Nursing</h3>
        <p class="bottom25">Post basic B.Sc. degree Nursing is a broad based education aimed to build upon the skills and competencies acquired at the Diploma in General Nursing and Midwifery level. It is specially directed to the upgrading of critical thinking skills, competencies and standards required for practice of professional nursing and midwifery and to prepare graduates to assume responsibilities as competent nurses and midwives in providing promotive, preventive curative rehabilitative services, who can make independent decisions in nursing situations, protect the rights of individuals in pursuit of health and conduct research studies in the area of nursing practice. They are also expected to assume the role of teacher, supervisor, researcher and manager in a clinical/ public health setting.
        </p>
        <h3 class="top30 bottom20">Best Practices</h3>
        <ul class="bottom25">
          <li> 	Small Group Learning</li>
          <li> 	Education Workshop</li>
          <li> 	Research Methodology Workshop</li>
          <li> 	Individual Research Project</li>
          <li> 	Inter-professional community practice</li>
        </ul>
        <h3 class="top30 bottom20">Curriculum and Year Plan</h3>
        <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
          <div class="panel panel-default">
            <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
            <div class="panel-heading" role="tab" id="headingOne">
              <h4 class="panel-title">
                  Curriculum
              </h4>
            </div>
          </a>
            <div id="collapseOne" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingOne">
              <div class="panel-body">
                <table class="table table-striped table-bordered table-hover">
                  <tbody>
                    <tr>
                  <td colspan="5">
                  <p><strong>First Year</strong></p>
                  </td>
                  </tr>
                  <tr>
                  <td colspan="2">
                  <p><strong>S.No</strong></p>
                  </td>
                  <td>
                  <p><strong>Subject</strong></p>
                  </td>
                  <td>
                  <p><strong>Hours Theory</strong></p>
                  <p>&nbsp;</p>
                  </td>
                  <td>
                  <p><strong>Hours Practical </strong></p>
                  <p>&nbsp;</p>
                  </td>
                  </tr>
                  <tr>
                  <td colspan="2">
                  <p>1.</p>
                  </td>
                  <td>
                  <p>Nursing Foundation</p>
                  </td>
                  <td>
                  <p>45</p>
                  </td>
                  <td>
                  <p>-</p>
                  </td>
                  </tr>
                  <tr>
                  <td colspan="2">
                  <p>2.</p>
                  </td>
                  <td>
                  <p>Nutrition &amp; Dietetics</p>
                  </td>
                  <td>
                  <p>30</p>
                  </td>
                  <td>
                  <p>15</p>
                  </td>
                  </tr>
                  <tr>
                  <td colspan="2">
                  <p>3.</p>
                  </td>
                  <td>
                  <p>Biochemistry &amp; Biophysics</p>
                  </td>
                  <td>
                  <p>60</p>
                  </td>
                  <td>
                  <p>-</p>
                  </td>
                  </tr>
                  <tr>
                  <td colspan="2">
                  <p>4.</p>
                  </td>
                  <td>
                  <p>Psychology</p>
                  </td>
                  <td>
                  <p>60</p>
                  </td>
                  <td>
                  <p>15</p>
                  </td>
                  </tr>
                  <tr>
                  <td colspan="2">
                  <p>5.</p>
                  </td>
                  <td>
                  <p>Maternal Nursing</p>
                  </td>
                  <td>
                  <p>60</p>
                  </td>
                  <td>
                  <p>240</p>
                  </td>
                  </tr>
                  <tr>
                  <td colspan="2">
                  <p>6.</p>
                  </td>
                  <td>
                  <p>Child Health Nursing</p>
                  </td>
                  <td>
                  <p>60</p>
                  </td>
                  <td>
                  <p>240</p>
                  </td>
                  </tr>
                  <tr>
                  <td colspan="2">
                  <p>7.</p>
                  </td>
                  <td>
                  <p>Microbiology</p>
                  </td>
                  <td>
                  <p>60</p>
                  </td>
                  <td>
                  <p>30</p>
                  </td>
                  </tr>
                  <tr>
                  <td colspan="2">
                  <p>8.</p>
                  </td>
                  <td>
                  <p>Medical Surgical Nursing</p>
                  </td>
                  <td>
                  <p>90</p>
                  </td>
                  <td>
                  <p>270</p>
                  </td>
                  </tr>
                  <tr>
                  <td colspan="2">
                  <p>9.</p>
                  </td>
                  <td>
                  <p>English (Qualifying)</p>
                  </td>
                  <td>
                  <p>60</p>
                  </td>
                  <td>
                  <p>-</p>
                  </td>
                  </tr>
                  <tr>
                  <td colspan="3">
                  <p><strong>Total Hours</strong></p>
                  </td>
                  <td>
                  <p><strong>525</strong></p>
                  </td>
                  <td>
                  <p><strong>810</strong></p>
                  </td>
                  </tr>
                  <tr>
                  <td colspan="5">
                  <p><strong>Second Year</strong></p>
                  </td>
                  </tr>
                  <tr>
                  <td>
                  <p>10.</p>
                  </td>
                  <td colspan="2">
                  <p>Sociology</p>
                  </td>
                  <td>
                  <p>60</p>
                  </td>
                  <td>
                  <p>-</p>
                  </td>
                  </tr>
                  <tr>
                  <td>
                  <p>11.</p>
                  </td>
                  <td colspan="2">
                  <p>Community Health Nursing</p>
                  </td>
                  <td>
                  <p>60</p>
                  </td>
                  <td>
                  <p>240</p>
                  </td>
                  </tr>
                  <tr>
                  <td>
                  <p>12.</p>
                  </td>
                  <td colspan="2">
                  <p>Mental Health Nursing</p>
                  </td>
                  <td>
                  <p>60</p>
                  </td>
                  <td>
                  <p>240</p>
                  </td>
                  </tr>
                  <tr>
                  <td>
                  <p>13.</p>
                  </td>
                  <td colspan="2">
                  <p>Introduction to Nursing Education</p>
                  </td>
                  <td>
                  <p>60</p>
                  </td>
                  <td>
                  <p>75</p>
                  </td>
                  </tr>
                  <tr>
                  <td>
                  <p>14.</p>
                  </td>
                  <td colspan="2">
                  <p>Introduction to Nursing Administration</p>
                  </td>
                  <td>
                  <p>60</p>
                  </td>
                  <td>
                  <p>180</p>
                  </td>
                  </tr>
                  <tr>
                  <td>
                  <p>15.</p>
                  </td>
                  <td colspan="2">
                  <p>Introduction to Nursing Research &amp; Statistics</p>
                  </td>
                  <td>
                  <p>45</p>
                  </td>
                  <td>
                  <p>120</p>
                  </td>
                  </tr>
                  <tr>
                  <td colspan="3">
                  <p><strong>Total Hours</strong></p>
                  </td>
                  <td>
                  <p><strong>345</strong></p>
                  </td>
                  <td>
                  <p><strong>855</strong></p>
                  </td>
                  </tr>
                  </tbody>
                  </table>
              </div>
            </div>
          </div>
          <div class="panel panel-default">
            <a class="collapsed" role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">
            <div class="panel-heading" role="tab" id="headingTwo">
              <h4 class="panel-title">
                  Year Plan
              </h4>
            </div>
          </a>
            <div id="collapseTwo" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingTwo">
              <div class="panel-body">
                <table class="table table-striped table-bordered table-hover">
                    <tbody>
                    <tr>
                    <td>
                    <p><strong>MONTH</strong></p>
                    </td>
                    <td>
                    <p><strong>DATE</strong></p>
                    </td>
                    <td>
                    <p><strong>EVENTS</strong></p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="2">
                    <p>September 2016</p>
                    </td>
                    <td>
                    <p>01.09.2016</p>
                    </td>
                    <td>
                    <p>University Examination Commences for all courses</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>05.09.2016</p>
                    </td>
                    <td>
                    <p>Teacher&rsquo;s Day</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="3">
                    <p>October</p>
                    <p>2016</p>
                    </td>
                    <td>
                    <p>03.10.2016 to 15.10.2016</p>
                    </td>
                    <td>
                    <p>Foundation course commences for B.Sc(N) I year</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>07.10.2016</p>
                    </td>
                    <td>
                    <p>State Level Workshop - CHN</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>26.10.2016</p>
                    </td>
                    <td>
                    <p>Continuing Nursing Education-MHN</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="4">
                    <p>November 2016</p>
                    </td>
                    <td>
                    <p>01.11.2016 to 05.11.2016</p>
                    </td>
                    <td>
                    <p>Newborn Week</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>16.11.2016</p>
                    </td>
                    <td>
                    <p>Guest Lecture - FON</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>18.11.2016</p>
                    </td>
                    <td>
                    <p>Founders Day</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>30.11.2016</p>
                    </td>
                    <td>
                    <p>Lamp Lighting Ceremony</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="3">
                    <p>December 2016</p>
                    </td>
                    <td>
                    <p>01.12.2016</p>
                    </td>
                    <td>
                    <p>World AIDS Day - Rally</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>02.12.2016</p>
                    </td>
                    <td>
                    <p>First Internal Examination Commences</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>20.12.2016</p>
                    </td>
                    <td>
                    <p>Guest Lecture - MSN</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>January-2017</p>
                    </td>
                    <td>
                    <p>27.01.2017</p>
                    </td>
                    <td>
                    <p>Continuing Nursing Education - MSN</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="3">
                    <p>February</p>
                    <p>2017</p>
                    </td>
                    <td>
                    <p>03.02.2017</p>
                    </td>
                    <td>
                    <p>National Level Conference - MSN</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>10.02.2016</p>
                    </td>
                    <td>
                    <p>District Level hands on workshop - PDN</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>17.02.2017</p>
                    </td>
                    <td>
                    <p>Guest Lecture- MHN</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="11">
                    <p>March-2017</p>
                    </td>
                    <td>
                    <p>08.03.2017</p>
                    </td>
                    <td>
                    <p>International Women&rsquo;s day</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>13.03.2017</p>
                    </td>
                    <td>
                    <p>Second Terminal Examination Commences</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>15.03.2017</p>
                    </td>
                    <td>
                    <p>Continuing Nursing Education -CHN</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>18.03.2017</p>
                    </td>
                    <td>
                    <p>Convocation Day</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>22.03.2017</p>
                    </td>
                    <td>
                    <p>In- Service Education</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>29.03.2017</p>
                    </td>
                    <td>
                    <p>Guest lecture-PDN</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>28.03.2017 to</p>
                    <p>30.03.2017</p>
                    </td>
                    <td>
                    <p>Education tour</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>06.04.2017</p>
                    </td>
                    <td>
                    <p>Safe motherhood Day</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>07.04.2017</p>
                    </td>
                    <td>
                    <p>WHO day</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>12.04.2017</p>
                    </td>
                    <td>
                    <p>Guest Lecture&nbsp; -CHN</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>17.04.2017</p>
                    </td>
                    <td>
                    <p>Continuing Nursing Education - OBGN</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="5">
                    <p>May</p>
                    <p>2017</p>
                    </td>
                    <td>
                    <p>08.05.2017</p>
                    </td>
                    <td>
                    <p>Mother&rsquo;s Day</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>10.05.2017</p>
                    </td>
                    <td>
                    <p>Continuing Nursing Education-PDN</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>12.05.2017</p>
                    </td>
                    <td>
                    <p>Nurses Day</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>01.06.2017</p>
                    </td>
                    <td>
                    <p>Third Terminal Examination commences</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>17.06.2017</p>
                    </td>
                    <td>
                    <p>Guest Lecture - OBGN</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="4">
                    <p>July</p>
                    <p>2017</p>
                    </td>
                    <td>
                    <p>11.07.2017</p>
                    </td>
                    <td>
                    <p>Model Examination commences</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>22.07.2017 &amp; 23.07.2016</p>
                    </td>
                    <td>
                    <p>International Nursing Conference</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>26.07.2017</p>
                    </td>
                    <td>
                    <p>Carrier orientation Programme</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>29.07.2017</p>
                    </td>
                    <td>
                    <p>Continuing Nursing Education-CHN</p>
                    </td>
                    </tr>
                    <tr>
                    <td rowspan="2">
                    <p>August</p>
                    <p>2017</p>
                    <p>&nbsp;</p>
                    </td>
                    <td>
                    <p>01.08.2017 to 08.08.2017</p>
                    </td>
                    <td>
                    <p>World Breast Feeding Celebration</p>
                    </td>
                    </tr>
                    <tr>
                    <td>
                    <p>04.08.2017</p>
                    </td>
                    <td>
                    <p>Guest lecture-MHN</p>
                    </td>
                    </tr>
                    </tbody>
                    </table>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>
<!-- Courses -->
